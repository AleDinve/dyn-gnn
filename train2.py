...
def model_training(seq_len, hidden_gnn, hidden_rnn, it, conv_type = 'gin'):
    raw_data = []
    ...
    model = DYN_GNN(input_gnn=1, hidden_gnn=hidden_gnn, output_gnn=hidden_rnn, layers_gnn=6, 
                    hidden_rnn=hidden_rnn, sequence_length=seq_len, device = device, conv_type= conv_type)
    ...
    for epoch in range(1, epochs + 1):
        ...
        raw_data.append({'Epoch': epoch, 'Train loss': train_loss, 
                         ...
                         'train accuracy':total_correct, 'iteration':it,'conv_type':conv_type})
    return raw_data
...
def main():
    ...
    hidden_list = [1,4,8]
    hidden_rnn = 8
    conv_type_list = ['gat','gin','gconv_mean', 'gconv_add','gcn']
        ...
        for seq_len in range(4,6):
            ...
            for hidden_gnn in hidden_list:
                ...
                for it in range(0,10):
                    ...
                    raw_data += model_training(seq_len, hidden_gnn, hidden_rnn, it, conv_type = conv_type)
            data = pd.DataFrame.from_records(raw_data)
        data.to_csv('dynamics_100124_single_hidden_T'+str(seq_len)+'.csv')
