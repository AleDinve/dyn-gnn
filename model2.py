from torch_geometric.nn import GraphConv, global_add_pool, GINConv, SAGEConv, GCNConv, GATConv
import torch
    ...
    def __init__(self, input_gnn, hidden_gnn, output_gnn, layers_gnn, 
        ...
        for _ in range(sequence_length):
            ...
            for _ in range(layers_gnn):
                if conv_type == 'gconv_mean':
                    self.gnns[-1].append(GraphConv(input_gnn, hidden_gnn, aggr='mean', bias=True).to(device))
                elif conv_type == 'graph_add':
                    self.gnns[-1].append(GraphConv(input_gnn, hidden_gnn, aggr='add', bias=True).to(device)) 
                elif conv_type == 'graphsage':
                    self.gnns[-1].append(SAGEConv(input_gnn, hidden_gnn, aggr='mean', bias=True).to(device))
                elif conv_type == 'gcn':
                    self.gnns[-1].append(GCNConv(input_gnn,hidden_gnn, bias=True).to(device))
                elif conv_type == 'gat':
                    self.gnns[-1].append(GATConv(input_gnn,hidden_gnn, bias=True).to(device))
                else:
